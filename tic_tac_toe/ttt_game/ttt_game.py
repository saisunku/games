...
class tic_tac_toe():
	...
	def __init__(self, board_sz, ai_player = None, blank_char = None):
		"""Initialize with a board of a given size and the character of the AI player if there is one"""
		self.board_sz = board_sz
		self.blank_char = blank_char
		self.board = [[self.blank_char for j in range(board_sz)] for k in range(board_sz)]
	...
	def get_winner(self, board = None):
		...
		# Return the player id if there is a winner and None otherwise
			if len(set(char_list)) == 1 and char_list[0] != self.blank_char:
				return char_list[0]
			else:
				return None
	...
	def is_full(self, board = None):
		...
		return not self.blank_char in flattened_board
	...
	def get_sym_equiv(self, board):
		...
		# MIR_VERT
		MIR_VERT = [[self.blank_char for j in range(self.board_sz)] for k in range(self.board_sz)]
		...
		# MIR_HORIZ
		MIR_HORIZ = [[self.blank_char for j in range(self.board_sz)] for k in range(self.board_sz)]
		for j in range(len(board)):
		...
		MIR_DIAG = [[self.blank_char for j in range(self.board_sz)] for k in range(self.board_sz)]
		for j in range(len(board)):
			for k in range(len(board)):
		...
		MIR_DIAG2 = [[self.blank_char for j in range(self.board_sz)] for k in range(self.board_sz)]
		for j in range(len(board)):
			for k in range(len(board)):
		...
		# ROT90
		ROT90 = [[self.blank_char for j in range(self.board_sz)] for k in range(self.board_sz)]
		for j in range(len(board)):
			for k in range(len(board)):
		...
		# ROT180
		ROT180 = [[self.blank_char for j in range(self.board_sz)] for k in range(self.board_sz)]
		for j in range(len(board)):
			for k in range(len(board)):
		...
		# ROT270
		ROT270 = [[self.blank_char for j in range(self.board_sz)] for k in range(self.board_sz)]
		for j in range(len(board)):
			for k in range(len(board)):
		...
		# ROT360 = [[self.blank_char for j in range(self.board_sz)] for k in range(self.board_sz)]
		#
		# for j in range(len(board)):
		#	for k in range(len(board)):
	...
	def random_ai(self, player):
		...
		avail_sqr = [idx for (idx, sqr) in enumerate(flattened_board) if sqr == self.blank_char]
